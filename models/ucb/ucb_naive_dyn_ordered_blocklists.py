...
from ucb_naive_dyn_blocklists import UCBNaiveDynamicDate, UCBNaiveDynParserOptions

np.seterr(divide="ignore", invalid="ignore")


class UCBNaiveDynamicOrderedDate(UCBNaiveDynamicDate):
    """
    ...
    def get_blocklist_coverage(self) -> float:
        if self._current_date not in self.blocklist_unique_count:
            self.set_blocklist_unique_counts_based_on_action_space()
        return round(len(self.blocklist_targets_found[self._current_date]) / float(
            self.blocklist_unique_count[self._current_date]), 2)
    ...
    def step(self):
        ...
        if self.current_epoch_num > 0 and \
            ...
            # at each new date, we wake up all nodes in the action space and also clear self.selected_target_count
            # or else we won't put targets to sleep correctly
            self.set_next_blocklist_date()
            self.init_blockers()
            self.action_space.wake_up_all_nodes()
            self.selected_target_count.clear()
            # resets more like the exploration_epoch_num and action attempts
            # that makes the calculation of UCB start from beginning while keeping Q-value
            if self.reset_more_per_date_threshold:
                self.exploration_epoch_num = 0
...
class UCBNaiveOrderedParserOptions(UCBNaiveDynParserOptions):
...
if __name__ == "__main__":
    ...
    run_multiprocessing(UCBNaiveDynamicOrderedDate, params, addition_kwargs=addition_kwargs)
