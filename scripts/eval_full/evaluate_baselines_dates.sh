...
# HOME="/path/to/home" bash scripts/eval_full/evaluate_baselines_dates.sh -e 1 -m 10000 -g gfwatch -s "top10k" -d 100 -t 1
...
outputdir=$RLFOLDER/models/outputs/baselines_with_dates_${size}_${ground_truth_option}_${timestamp}
mkdir -p ${outputdir}
...
cd $RLFOLDER/scripts
python3 plotter_baselines_dates_only.py --output_file_name baselines_dates_grid.pdf --output_directory $outputdir --ground_truth_file_path $ground_truth --measurements $measurements --episodes $episodes --action_space_file_path $tranco_file --per_date_threshold $per_date_threshold --selected_target_max_try $selected_target_max_try
