...
# EXAMPLE RUN: HOME="/path/to/home" bash scripts/eval_full/evaluate_thompson_sampling.sh -e 1 -m 1000 -g russia -p "true" -f "categories" -s "top10k" -a "thompson_sampling" -r "false"
RLFOLDER=$HOME/cenrl
...
if [[ $longitudinal == "static" ]]; then
    python3 $modelpath $sample_by_target_rank -np $num_of_processes_for_episodes -m $measurements -E $episodes -o ${outputdir}/${algorithm}_eps${epsilon}_stepsize${stepsize}_initval${initq} -g ${ground_truth} -a ${tranco_file} -f ${features[@]} 2>&1 &
elif [[ $longitudinal == "dyn_blocklists" ]]; then
    python3 $modelpath $sample_by_target_rank -np $num_of_processes_for_episodes $action_space_multi_parents -m $measurements -E $episodes -o ${outputdir}/${algorithm}_eps${epsilon}_stepsize${stepsize}_initval${initq} -g ${ground_truth} -a ${tranco_file} -f ${features[@]} --selected_target_max_try ${selected_target_max_try} 2>&1 &
elif [[ $longitudinal == "dyn_ordered" ]]; then
    python3 $modelpath $sample_by_target_rank -np $num_of_processes_for_episodes $reset_more_per_date_threshold $action_space_multi_parents -m $measurements -E $episodes -o ${outputdir}/${algorithm}_eps${epsilon}_stepsize${stepsize}_initval${initq} -g ${ground_truth} -a ${tranco_file} -f ${features[@]} -pdt ${per_date_threshold} --selected_target_max_try ${selected_target_max_try} 2>&1 &
else
echo "Invalid long"
fi
