...
#EXAMPLE RUN: HOME="/path/to/home" bash scripts/eval_full/evaluate_ucb_naive_test.sh -e 2 -m 1000 -g gfwatch -f "categories" -s "top10k" -a "ucb_naive" -r "false"
RLFOLDER=$HOME/cenrl
...
# Nested for loop
for ucb_c in 0.03 0.3
do
  # ucb_c loop
  for stepsize in 0.0 0.2 
  do
    # stepsize loop
    for initq in 0.0 0.2
    do
      if [[ $longitudinal == "static" ]]; then
        python3 $modelpath $sample_by_target_rank -np $num_of_processes_for_episodes -m $measurements -E $episodes -c=$ucb_c -s $stepsize -V $initq -o ${outputdir}/${algorithm}_c${ucb_c}_stepsize${stepsize}_initval${initq} -g ${ground_truth} -a ${tranco_file} -f ${features[@]} 2>&1 &
      elif [[ $longitudinal == "dyn_blocklists" ]]; then
        python3 $modelpath $sample_by_target_rank -np $num_of_processes_for_episodes $action_space_multi_parents -m $measurements -E $episodes -c=$ucb_c -s $stepsize -V $initq -o ${outputdir}/${algorithm}_c${ucb_c}_stepsize${stepsize}_initval${initq} -g ${ground_truth} -a ${tranco_file} -f ${features[@]} --selected_target_max_try ${selected_target_max_try} 2>&1 &
      elif [[ $longitudinal == "dyn_ordered" ]]; then
        python3 $modelpath $sample_by_target_rank -np $num_of_processes_for_episodes $reset_more_per_date_threshold $action_space_multi_parents -m $measurements -E $episodes -c=$ucb_c -s $stepsize -V $initq -o ${outputdir}/${algorithm}_c${ucb_c}_stepsize${stepsize}_initval${initq} -g ${ground_truth} -a ${tranco_file} -f ${features[@]} -pdt ${per_date_threshold} --selected_target_max_try ${selected_target_max_try} 2>&1 &
      else
        echo "Invalid long"
      fi
    done
    echo "running with (c=${ucb_c},stepsize=${stepsize},initq=${initq})"
  done
done
